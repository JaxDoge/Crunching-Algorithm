...
# For this question, we don't need value. So we can simplify the solution
...
class TrieTree:
	...
	def put(self, key):
		...
		for c in key:
			chidx = ord(c) - ord('a')
			if not p.children[chidx]:
				p.children[chidx] = TrieNode()
			p = p.children[chidx]
		p.is_end = True
	
	def hasKeyWithPattern(self, pattern):
		return self.hKWPHelper(self.root, pattern, 0)

	def hKWPHelper(self, node, pattern, index):
		if not node:
			return False
		...
		if c == '.':
			for chld in node.children:
				if self.hKWPHelper(chld, pattern, index + 1):
					return True
			return False

		else:
			return self.hKWPHelper(node.children[ord(c) - ord('a')], pattern, index + 1)
...
class WordDictionary:
	...
	def addWord(self, word: str) -> None:
		self.trie_tree.put(word)

	def search(self, word: str) -> bool:
		return self.trie_tree.hasKeyWithPattern(word)